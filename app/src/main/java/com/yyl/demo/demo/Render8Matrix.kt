 ...
 *
 */
class Render8Matrix(val context: Context) : Renderer {
...
//            0f, 1f, 1f, 1f, 0f, 0f, 1f, 0f
    ...
    private var imgTextureId2: Int = 0
    private var imgTextureId3: Int = 0
...
//---------------------FBO（帧缓冲对象）---------------------
        ...
        imgTextureId2 = loadTexrute(R.drawable.demo3)
        imgTextureId3 = loadTexrute(R.drawable.demo)
...
//        3、使用
        ...
        GLES20.glEnableVertexAttribArray(vPosition)
        //第一个图顶点 占了32个字节  因为vertexData.size * 4=顶点*2*4个字节     4个顶点=32字节  8个顶点=64字节   所以偏移量offset=32
        ...
        //绘制第三张图片  三角形
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, imgTextureId3)

        GLES20.glEnableVertexAttribArray(vPosition)
        //第一个图顶点 占了32个字节  因为vertexData.size * 4=顶点*2*4个字节    4个顶点=32字节  8个顶点=64字节   所以偏移量offset=64
